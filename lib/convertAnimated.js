...
// Tries the given ffmpeg arg arrays in order, returns the first that exits 0.
// The full ffmpeg log of the last attempted run is appended to `logSink`.
const FFMPEG_ATTEMPTS = [
  // Default: H.264 high profile, even-dim scale, no audio
  (inp, out) => ([
    '-i', inp,
    '-c:v', 'libx264',
    '-pix_fmt', 'yuv420p',
  ...
  // Fallback 1: drop high profile (some libx264 wasm builds reject it)
  (inp, out) => ([
    '-i', inp,
    '-c:v', 'libx264',
    '-pix_fmt', 'yuv420p',
  ...
  // Fallback 2: force input format + assume 25fps (helps with malformed timing)
  (inp, out) => ([
    '-f', 'webp_pipe',
    '-i', inp,
    '-c:v', 'libx264',
...
function tailLog(logLines, n = 4) {
  return logLines
    .filter((l) => typeof l === 'string' && l && !/^frame=\s*\d/.test(l))
    .slice(-n)
    .join(' | ')
    .slice(0, 280);
}
...
export async function convertAnimatedToMp4(file, { onProgress } = {}) {
  ...
  const fatal = async (kind, err) => {
    // True engine death (worker thrown, not just non-zero exit) — recycle it.
    // eslint-disable-next-line no-console
    console.error(`[ffmpeg ${kind}] worker error`, err, '\n=== ffmpeg log ===\n' + logLines.join('\n'));
    await resetFFmpeg();
    const wrapped = new Error(MSG_CONVERT_FAIL);
    wrapped.detail = (err && err.message ? `${kind}: ${err.message}` : kind);
    return wrapped;
  };
  ...
  try {
    ...
    try {
      ...
      throw await fatal('writeFile', err);
    }

    let lastExit = null;
    for (let i = 0; i < FFMPEG_ATTEMPTS.length; i++) {
      logLines.length = 0;
      let exitCode;
      try {
        exitCode = await ffmpeg.exec(FFMPEG_ATTEMPTS[i](inputName, outputName));
      } catch (err) {
        // exec throwing = worker error, abandon retries.
        throw await fatal('exec', err);
      }
      ...
      lastExit = exitCode;
      // eslint-disable-next-line no-console
      console.warn(`[ffmpeg exec] attempt ${i + 1}/${FFMPEG_ATTEMPTS.length} failed (exit ${exitCode}). log tail: ${tailLog(logLines)}`);
      // Remove any partial output so the next attempt has a clean slate.
      try { await ffmpeg.deleteFile(outputName); } catch (_) { /* ignore */ }
    }

    if (lastExit !== 0) {
      // All attempts failed but the engine is still alive — keep it.
      // eslint-disable-next-line no-console
      console.error('[ffmpeg exec] all attempts failed.\n=== ffmpeg log ===\n' + logLines.join('\n'));
      await cleanupFiles();
      const wrapped = new Error(MSG_CONVERT_FAIL);
      wrapped.detail = tailLog(logLines) || `exit ${lastExit}`;
      throw wrapped;
    }
    ...
    try {
      ...
      // readFile fail with exit==0 should be impossible — treat as fatal.
      throw await fatal('readFile', err);
    }
    ...
    try { if (typeof ffmpeg.off === 'function') ffmpeg.off('progress', progressHandler); } catch (_) { /* ignore */ }
    try { if (typeof ffmpeg.off === 'function') ffmpeg.off('log', logHandler); } catch (_) { /* ignore */ }
  }
}
