...
// libx264/yuv420p require even dimensions. Round DOWN (matches ffmpeg
// `scale=trunc(iw/2)*2`). Rounding up would exceed the source frame's
// codedWidth/codedHeight when those are odd, and VideoFrame's visibleRect
// constructor would throw "right N exceeds codedWidth N-1".
function evenDim(n) {
  return n - (n % 2);
}
...
export async function convertAnimatedToMp4(file, { onProgress } = {}) {
  ...
  try {
    ...
    // 2. Probe the first frame for dimensions. Use codedWidth/Height because
    //    `visibleRect` is bounded by those, and clamp the rounded-down even
    //    output to fit inside them as a safety net.
    let firstResult;
    try {
    ...
    const srcDW = firstResult.image.displayWidth;
    const srcDH = firstResult.image.displayHeight;
    const srcCW = firstResult.image.codedWidth;
    const srcCH = firstResult.image.codedHeight;
    firstResult.image.close();
    const width = Math.min(evenDim(srcDW), srcCW);
    const height = Math.min(evenDim(srcDH), srcCH);
    if (width < 2 || height < 2) {
      throw makeErr(MSG_DECODE_FAIL, new Error(`dimensions too small: ${srcDW}x${srcDH}`));
    }
    ...
    try {
      for (let i = 0; i < frameCount; i++) {
        ...
        // Always specify visibleRect — guarantees the frame dimensions match
        // what we passed to encoder.configure(), even when the rounded-down
        // dims trim a pixel off an odd-sized source.
        const frame = new VideoFrame(sourceImage, {
