...
// 1. Default worker resolution constructs a cross-origin classic Worker
//    which iOS silently rejects → we pre-fetch 814.ffmpeg.js as a Blob
//    URL and pass it as classWorkerURL (same-origin module worker).
// 2. fetch() + ReadableStream progress is flaky on iOS → we use XHR
//    which has battle-tested progress events.
// 3. unpkg.com transient failures → retry 3x then fall back to jsDelivr.
...
export async function loadFFmpeg() {
  ...
  loadPromise = (async () => {
    ...
    try {
      ...
      try {
        const workerPaths = CDN_BASES.ffmpeg.map((b) => `${b}/${FFMPEG_WORKER_CHUNK}`);
        classWorkerURL = await fetchWithFallback(workerPaths, 'text/javascript', 0, 0.03);
      } catch (err) {
      ...
      const coreURL = await fetchWithFallback(corePaths, 'text/javascript', 0.03, 0.08);

      reportStatus(STATUS.wasmDownloading);
      const wasmPaths = CDN_BASES.core.map((b) => `${b}/ffmpeg-core.wasm`);
      const wasmURL = await fetchWithFallback(wasmPaths, 'application/wasm', 0.08, 0.93);
