...
export class FileCard {
  constructor(template, file, callbacks) {
    ...
    this.elErrorDetail = node.querySelector('.card-error-detail');
  ...
  setState(state) {
    ...
    if (state === 'analyzing') {
      ...
      if (this.elErrorDetail) this.elErrorDetail.hidden = true;
  ...
  setError(message, detail = '') {
    ...
    if (this.elErrorDetail) {
      this.elErrorDetail.textContent = detail || '';
      this.elErrorDetail.hidden = !detail;
    }
  }
